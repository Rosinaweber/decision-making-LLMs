# NOTE: Before running this script, make sure to install the required libraries with:
# pip install pandas numpy tiktoken scikit-learn nltk anthropic openai
# The anthropic library is specifically needed for Claude API integration
# 
# This script is adapted from the GPT-4o implementation to use Claude 3.7 Sonnet
# while maintaining the same approach and metrics for comparison.

import random
import pandas as pd
from datetime import datetime
import openai  # Keep for embeddings
import anthropic  # Add for Claude API
import numpy as np
import tiktoken
# Removing pyemd dependency which has issues with numpy 2.x
...
anthropic_api_key = ""  # Replace with your actual key
...
# Body Part Pain Data and Prompt Generation (Restricted for 300 Instances)
...
# Computer Parts Data and Prompt Generation (Restricted for 300 Instances)
all_computer_brands = [
...
# Job Hiring Prompt Generation (Using Provided 300 Job Titles)
job_list = [
    "Academic Advisor",
    "Account Executive",
    "Accountant",
...
def ask_claude(messages, model="claude-3-7-sonnet-20250219"):
    """
    Convert OpenAI message format to Anthropic's format and get Claude's response.
    
    Note: This function maintains compatibility with the existing code by accepting
    the same message format as ask_gpt but converts it to Anthropic's format.
    """
    try:
        # Build the conversation history in a format Claude can understand
...
def run_car_experiment(n=10, save_file="claude-3-7-sonnet-car_responses5050.csv",
                       chat_model="claude-3-7-sonnet-20250219", emb_model="text-embedding-ada-002"):
    results = []
    for i in range(n):
        ...
        fix_response = ask_claude(messages, model=chat_model)
        ...
        explanation_response = ask_claude(messages, model=chat_model)
...
def run_body_experiment(n=10, save_file="claude-3-7-sonnet-body_responses50.csv",
                        chat_model="claude-3-7-sonnet-20250219", emb_model="text-embedding-ada-002"):
    results = []
    for i in range(n):
...
def run_computer_experiment(n=10, save_file="claude-3-7-sonnet-computer_responses50.csv",
                            chat_model="claude-3-7-sonnet-20250219", emb_model="text-embedding-ada-002"):
    results = []
    for i in range(n):
...
def run_job_experiment(n=10, save_file="claude-3-7-sonnet-job_responses-300.csv",
                       chat_model="claude-3-7-sonnet-20250219", emb_model="text-embedding-ada-002"):
    results = []
    for i in range(n):
...
if __name__ == "__main__":
    print("Running Claude 3.7 Sonnet tests with a few prompts from each category...")
    
    # Run car experiment with 3 prompts
    #print("\n=== Starting Car Repair Experiment ===")
    run_car_experiment(n=1)

    # Run body experiment with 3 prompts
    #print("\n=== Starting Body Pain Experiment ===")
    run_body_experiment(n=1)

    # Run computer experiment with 3 prompts
    #print("\n=== Starting Computer Problem Experiment ===")
    run_computer_experiment(n=1)

    # Run job hiring experiment with 3 prompts
    print("\n=== Starting Job Hiring Experiment ===")
